...
const LiveHistoryPage: React.FC = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 py-4 md:px-4 sm:p-0">
      {/* Header */}
      <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">Live History</h1>
        <p className="text-sm sm:text-base text-gray-600">View your past live session statistics</p>
      </div>
      ...
      {sessions.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm text-gray-600 mb-1">Total Viewers</p>
            ...
            <div className="md:hidden divide-y divide-gray-200">
              {sessions.map((session) => (
                <div key={session.id} className="p-4 space-y-3">
                  <h3 className="font-semibold text-gray-900 text-base mb-2">{session.title}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-700 font-medium">{formatDuration(session.durationMinutes)}</span>
                    </div>
                    
                    <div className="flex items-center gap-2 text-sm">
                      <Users className="w-4 h-4 text-blue-600" />
                      <span className="text-blue-600 font-semibold">{session.totalViewers}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="inline-flex items-center px-2 py-0.5 bg-purple-100 text-purple-700 text-sm font-semibold rounded-full">
                        {session.maxViewers}
                      </span>
                    </div>
